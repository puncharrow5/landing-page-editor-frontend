...
import * as S from "./MobileFooterBox.style";

interface Props {
  siteId: number;
  data?: MobileFooterEntity | null;
  setData: React.Dispatch<React.SetStateAction<SiteEntity | undefined>>;
  handleReset: () => void;
}

export const MobileFooterBox = ({ siteId, data, setData, handleReset }: Props) => {
  const client = useApolloClient();
  const { ToastMessage } = useToastMessage();

  const [open, setOpen] = useState<boolean>(false);

  const [loadUpdateMobileFooter, { loading }] = useUpdateMobileFooterMutation({
    onCompleted: () => {
